...
@section('content')
    ...
    <div class="row listSearch">
      ...
      @foreach ($item->configWaktuPengirimanLog as $configWaktuPengirimanLog)
        @foreach ($configWaktuPengirimanLog->transaksiApps as $transaksiApps)
          @if ($transaksiApps->status == 4 && $transaksiApps->tw4 >= date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . '-' . $configHariPengiriman->hari . 'days')) && $transaksiApps->tw4 <= date('Y-m-d H:i:s') && $transaksiApps->id_kota == $driver->id_kota && $transaksiApps->tipe_keranjang == 1)
      ...
      <div class="col-12 mt-3 text-center">
        ...
        <span class="d-block">Sedang tidak ada pesanan selesai</span>
      </div>
      @endif
    </div>
    {{-- Level 1 --}}
    ...
    <div class="row listSearch">
      ...
      @foreach ($item->configWaktuPengirimanLog as $configWaktuPengirimanLog)
        @foreach ($configWaktuPengirimanLog->transaksiApps as $transaksiApps)
          @if ($transaksiApps->status == 4 && $transaksiApps->tw4 >= date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . '-' . $configHariPengiriman->hari . 'days')) && $transaksiApps->tw4 <= date('Y-m-d H:i:s') && $transaksiApps->id_kota == $driver->id_kota && $transaksiApps->tipe_keranjang == 1 && $transaksiApps->id_driver == $driver->id_driver)
            <div class="col-12 mt-2 card-data">
              <div class="card">
                <div class="card-header bg-secondary text-white">
                  <div class="card-title">
                    ...
                    <div class="col-3">
                      @foreach ($transaksiApps->transaksiAppsDetail as $transaksiAppsDetail)
                          @if (count($transaksiAppsDetail->produkB2CHarga->produkB2C->produkSub->produkMaster->produkMasterGambar) > 0)
                            <img src="{{ $transaksiAppsDetail->produkB2CHarga->produkB2C->produkSub->produkMaster->produkMasterGambar[0]->gambar }}" alt="Gambar Produk" width="60" height="60">
                          @else
                            <img src="https://storage.googleapis.com/assets-warungsegar/images/defaultProduk.jpg" alt="GambarProduk" width="60" height="60">
                          @endif
                          @php
                              break;
                          @endphp
